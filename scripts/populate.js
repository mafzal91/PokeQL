  ...
  const k = (()=>{
    ...
    async.waterfall([
      (waterfallCallback) => {
        ...
        var funcs = Object.keys(routes).map(key => { //Go through all keys in routes

          return (next) => { //return function that async.series will run
            axios.get(routes[key]).then(res => { //Fetch route

              async.eachSeries(res.data.results, (item, eachCb) => { //for every url in the results
                let id = item.url.split('/')[6] //get the resource id (param on url)

                db.collection(key).findOne({id: parseInt(id,10)}, (err, res) => { //See if it exists on db
                  // console.log(`db.collection(${key}).findOne({id: ${id}})`)
                  if(!res){
                    async.retry({times:5, interval: 5000}, (retryCallback) => { //Fetch Resource if it doesnt and insert into db
                      console.log(moment().format('lll'), item.url)

                      axios.get(`${item.url}`).then(res => {
                        db.collection(key).insert(res.data, (err, result) => {
                          if (err) throw err;
                          console.log(`Inserted ${key} ${id} ${item.name}`)
                          setTimeout(() => retryCallback(err, res),5000)
                        });
                      }).catch(retryCallback)

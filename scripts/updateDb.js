...
mongo.connect(url, (err, db) => {
  var populateType = {
    berry: (y, next) => {
      async.parallel(
        {
          firmness: (cb) =>
            findOne(models["berryFirmness"], {name: y.firmness.name}, cb),
          item: (cb) => findOne(models["item"], {name: y.item.name}, cb),
          natural_gift_type: (cb) =>
            findOne(models["type"], {name: y.natural_gift_type.name}, cb),
          flavors: (cb) =>
            findMany(
              models["berryFlavor"],
              mapFind(y.flavors, "name", "flavor.name"),
              cb,
            ),
        },
    ...
    "berry-firmness": (y, next) => {
      async.parallel(
        {
          berries: (cb) =>
            findMany(models["berry"], mapFind(y.berries, "name", "name"), cb),
          names: (cb) =>
            findMany(
              models["language"],
              mapFind(y.names, "name", "language.name"),
              cb,
            ),
        },
        (err, {names, berries}) => {
      ...
      async.parallel(
        {
          berries: (cb) =>
            findMany(
              models["berry"],
              mapFind(y.berries, "name", "berry.name"),
              cb,
            ),
          names: (cb) =>
            findMany(
          ...
          contestType: (cb) =>
            findOne(models["contestType"], {name: y.contest_type.name}, cb),
        },
      ...
      async.parallel(
        {
          ...
          berryFlavor: (cb) =>
            findOne(models["berryFlavor"], {name: y.berry_flavor.name}, cb),
        },
      ...
      async.parallel(
        {
          effect_entries: (cb) =>
            findMany(
              models["language"],
              mapFind(y.effect_entries, "name", "language.name"),
              cb,
            ),
          flavor_text_entries: (cb) =>
            findMany(
              models["language"],
              mapFind(y.flavor_text_entries, "name", "language.name"),
              cb,
            ),
        },
          ...
          models["contestEffect"]
            ...
            .then((res) => {
              ...
              res.effect_entries = y.effect_entries.map((i) => ({
                language: effect_entries.find((k) => k.name === i.language.name)
                  ._id,
                effect: i.effect,
              }));
              res.flavor_text_entries = y.flavor_text_entries.map((i) => ({
                language: flavor_text_entries.find(
                  (k) => k.name === i.language.name,
                )._id,
    ...
    "super-contest-effect": (y, next) => {
      objectPromiseMixed({
        appeal: y.appeal,
        jam: y.jam,
        flavor_text_entries: Promise.all(
          y.flavor_text_entries.map((i) =>
            objectPromiseMixed({
        ...
        moves: findManyPromises["move"](mapFind(y.moves, "name", "name")).then(
          (res) => res.map((i) => i._id),
        ),
      }).then((res) => {
        findPromises["superContestEffect"]({pokeapi_id: y.id}).then(
          (entity) => {
    ...
    "encounter-condition-value": (y) => {
      return objectPromiseMixed({
        ...
        .then((res) => {
          return findPromises["encounterConditionValue"]({
            pokeapi_id: y.id,
          }).then((entity) => {
    ...
    "evolution-chain": (y, next) => {
      const mapChain = async (chain) => {
        return await Promise.all(
          chain.map((i) =>
            objectPromiseMixed({
              is_baby: i.is_baby,
              species: findOnePromise(models["pokemonSpecies"], {
                name: i.species.name,
              }).then((res) => res._id),
              evolves_to: mapChain(i.evolves_to),
              evolution_details: Promise.all(
                i.evolution_details.map((k) =>
                  objectPromiseMixed({
                    item: k.item
                      ? findPromises["item"]({name: k.item.name}).then(
                          (res) => res._id,
                        )
                      : null,
                    trigger: k.item
                      ? findPromises["evolutionTrigger"]({
                          name: k.trigger.name,
                        }).then((res) => res._id)
                      : null,
                    gender: k.gender,
                    held_item: k.held_item
                      ? findPromises["item"]({name: k.held_item.name}).then(
                          (res) => res._id,
                        )
                      : null,
                    known_move: k.known_move
                      ? findPromises["move"]({name: k.known_move.name}).then(
                          (res) => res._id,
                        )
                      : null,
                    known_move_type: k.known_move_type
                      ? findPromises["type"]({
                          name: k.known_move_type.name,
                        }).then((res) => res._id)
                      : null,
                    location: k.location
                      ? findPromises["location"]({name: k.location.name}).then(
                          (res) => res._id,
                        )
                      : null,
                    ...
                    party_species: k.party_species
                      ? findPromises["pokemonSpecies"]({
                          name: k.party_species.name,
                        }).then((res) => res._id)
                      : null,
                    party_type: k.party_type
                      ? findPromises["type"]({name: k.party_type.name}).then(
                          (res) => res._id,
                        )
                      : null,
                    relative_physical_stats: k.relative_physical_stats,
                    time_of_day: k.time_of_day,
                    trade_species: k.trade_species
                      ? findPromises["pokemonSpecies"]({
                          name: k.trade_species.name,
                        }).then((res) => res._id)
                      : null,
      ...
      let promises = {
        baby_trigger_item: y.baby_trigger_item
          ? findPromises["item"]({name: y.baby_trigger_item.name}).then(
              (res) => res._id,
            )
          : null,
    ...
    "evolution-trigger": (y, next) => {
      objectPromiseMixed({
        ...
        names: Promise.all(
          y.names.map((i) => {
            return objectPromiseMixed({
              language: findPromises["language"]({name: i.language.name}).then(
                (res) => res._id,
              ),
              name: i.name,
            });
        ...
        pokemon_species: findManyPromises["pokemonSpecies"](
          mapFind(y.pokemon_species, "name", "name"),
        ),
      }).then((res) => {
    ...
    generation: (y, next) => {
      objectPromiseMixed({
        name: y.name,
        abilities: findManyPromises["ability"](
          mapFind(y.abilities, "name", "name"),
        ),
        names: Promise.all(
          y.names.map((i) =>
            objectPromiseMixed({
        ...
        version_groups: findManyPromises["versionGroup"](
          mapFind(y.version_groups, "name", "name"),
        ),
      }).then((res) => {
    ...
    pokedex: (y, next) => {
      objectPromiseMixed({
        ...
        pokemon_entries: Promise.all(
          y.pokemon_entries.map((i) =>
            objectPromiseMixed({
              pokemon_species: findPromises["pokemonSpecies"]({
        ...
        region: y.region
          ? findPromises["region"]({name: y.region.name})
          : y.region,
        version_groups: findManyPromises["versionGroup"](
          mapFind(y.version_groups, "name", "name"),
        ),
    ...
    version: (y, next) => {
      objectPromiseMixed({
        ...
        version_group: findPromises["versionGroup"]({
          name: y.version_group.name,
        }),
      })
        .then((res) => {
    ...
    "version-group": (y, next) => {
      objectPromiseMixed({
        ...
        move_learn_methods: findManyPromises["moveLearnMethod"](
          mapFind(y.move_learn_methods, "name", "name"),
        ),
        pokedexes: findManyPromises["pokedex"](
          mapFind(y.pokedexes, "name", "name"),
        ),
        regions: findManyPromises["region"](mapFind(y.regions, "name", "name")),
        versions: findManyPromises["version"](
          mapFind(y.versions, "name", "name"),
        ),
      })
        .then((res) => {
    ...
    item: (y) => {
      if (y.baby_trigger_for == null) return;
        ...
        return objectPromiseMixed({
          ...
          fling_effect: y.fling_effect
            ? findPromises["itemFlingEffect"]({name: y.fling_effect.name})
            : null,
          attributes: findManyPromises["itemAttribute"](
            mapFind(y.attributes, "name", "name"),
          ),
          category: findPromises["itemCategory"]({name: y.category.name}),
          effect_entries: Promise.all(
            y.effect_entries.map((i) =>
              objectPromiseMixed({
                effect: i.effect,
          ...
          flavor_text_entries: Promise.all(
            y.flavor_text_entries.map((i) =>
              objectPromiseMixed({
                ...
                language: findPromises["language"]({name: i.language.name}),
                version_group: findPromises["versionGroup"]({
                  name: i.version_group.name,
                }),
              }),
            ),
          ),
          game_indices: Promise.all(
            y.game_indices.map((i) =>
              objectPromiseMixed({
                game_index: i.game_index,
                generation: findPromises["generation"]({
                  name: i.generation.name,
                }),
              }),
            ),
          ),
          ...
          sprites: y.sprites,
          held_by_pokemon: Promise.all(
            y.held_by_pokemon.map((i) =>
              objectPromiseMixed({
                pokemon: findPromises["pokemon"]({name: i.pokemon.name}),
                version_details: Promise.all(
                  i.version_details.map((k) =>
                    objectPromiseMixed({
                      rarity: k.rarity,
          ...
          baby_trigger_for: y.baby_trigger_for
            ? findPromises["evolutionChain"]({
                pokeapi_id: getApiId(y.baby_trigger_for.url),
              }).then((i) => i._id)
            : null,
          machines: Promise.all(
            y.machines.map((i) =>
              objectPromise({
                machine: findPromises["machine"]({
                  pokeapi_id: getApiId(i.machine.url),
                }),
                version_group: findPromises["versionGroup"]({
                  name: i.version_group.name,
                }),
    ...
    "item-pocket": (y, next) => {
      objectPromiseMixed({
        name: y.name,
        categories: findManyPromises["itemCategory"](
          mapFind(y.categories, "name", "name"),
        ),
        names: Promise.all(
          y.names.map((i) =>
            objectPromiseMixed({
    ...
    move: (y, next) => {
      objectPromiseMixed({
        ...
        priority: y.priority,
        power: y.power,
        contest_combos: y.contest_combos
          ? objectPromiseMixed({
              normal: objectPromiseMixed({
                use_before: y.contest_combos.normal.use_before
                  ? findManyPromises["move"](
                      mapFind(
                        y.contest_combos.normal.use_before,
                        "name",
                        "name",
                      ),
                    )
                  : [],
                use_after: y.contest_combos.normal.use_after
                  ? findManyPromises["move"](
                      mapFind(
                        y.contest_combos.normal.use_after,
                        "name",
                        "name",
                      ),
                    )
              ...
              super: objectPromiseMixed({
                use_before: y.contest_combos.super.use_before
                  ? findManyPromises["move"](
                      mapFind(
                        y.contest_combos.super.use_before,
                        "name",
                        "name",
                      ),
                    )
                  : [],
                use_after: y.contest_combos.super.use_after
                  ? findManyPromises["move"](
                      mapFind(y.contest_combos.super.use_after, "name", "name"),
                    )
                  : [],
              }),
            })
          : y.contest_combos,
        contest_type: y.contest_type
          ? findPromises["contestType"]({name: y.contest_type.name})
          : y.contest_type,
        contest_effect: y.contest_effect
          ? findPromises["contestEffect"]({name: y.contest_effect.name})
          : y.contest_effect,
        damage_class: y.damage_class
          ? findPromises["moveDamageClass"]({name: y.damage_class.name})
          : y.damage_class,
        effect_entries: Promise.all(
          y.effect_entries.map((i) =>
            objectPromiseMixed({
        ...
        effect_changes: Promise.all(
          y.effect_changes.map((i) =>
            objectPromiseMixed({
              version_group: findPromises["versionGroup"]({
                name: i.version_group.name,
              }),
              effect_entries: Promise.all(
                i.effect_entries.map((k) =>
                  objectPromiseMixed({
        ...
        meta: y.meta
          ? objectPromiseMixed({
              ailment: y.meta.ailment
                ? findPromises["moveAilment"]({name: y.meta.ailment.name})
                : y.meta.ailment,
              category: y.meta.category
                ? findPromises["moveCategory"]({name: y.meta.category.name})
                : y.meta.category,
        ...
        stat_changes: Promise.all(
          y.stat_changes.map((i) =>
            objectPromiseMixed({
              change: i.change,
        ...
        super_contest_effect: y.super_contest_effect
          ? findPromises["superContestEffect"]({
              name: y.super_contest_effect.name,
            })
          : y.super_contest_effect,
        target: y.target
          ? findPromises["moveTarget"]({name: y.target.name})
          : y.target,
    ...
    location: (y, next) => {
      objectPromiseMixed({
        ...
        areas: findManyPromises["locationArea"](
          mapFind(y.areas, "name", "name"),
        ),
      })
        .then((res) => {
    ...
    "location-area": (y) => {
      ...
      return objectPromiseMixed({
        ...
        encounter_method_rates: Promise.all(
          y.encounter_method_rates.map((i) =>
            objectPromiseMixed({
              encounter_method: findPromises["encounterMethod"]({
                name: i.encounter_method.name,
              }),
              version_details: Promise.all(
                i.version_details.map((k) =>
                  objectPromiseMixed({
        ...
        location: findPromises["location"]({
          pokeapi_id: getApiId(y.location.url),
        }).then((res) => {
          console.log("region", res);
          return res._id;
        }),
        names: populateNames(y.names),
        pokemon_encounters: Promise.all(
          y.pokemon_encounters.map((i) =>
            objectPromiseMixed({
              pokemon: findPromises["pokemon"]({name: i.pokemon.name}),
              version_details: Promise.all(
                i.version_details.map((k) =>
                  objectPromiseMixed({
                    ...
                    encounter_details: Promise.all(
                      k.encounter_details.map((x) =>
                        objectPromiseMixed({
                          min_level: x.min_level,
                          max_level: x.max_level,
                          condition_values: findManyPromises[
                            "encounterConditionValue"
                          ](mapFind(x.condition_values, "name", "name")),
                          chance: x.chance,
                          method: findPromises["encounterMethod"]({
                            name: x.method.name,
                          }),
                        }),
                      ),
        ...
        .then((res) => {
          ...
          return findPromises["locationArea"]({pokeapi_id: y.id}).then(
            (entity) => {
              return entity.set(res).save();
            },
          );
    ...
    region: (y, next) => {
      objectPromiseMixed({
        name: y.name,
        locations: findManyPromises["location"](
          mapFind(y.locations, "name", "name"),
        ),
        main_generation: findPromises["generation"]({
          name: y.main_generation.name,
        }),
        names: populateNames(y.names),
        pokedexes: findManyPromises["pokedex"](
          mapFind(y.pokedexes, "name", "name"),
    ...
    gender: (y, next) => {
      objectPromiseMixed({
        name: y.name,
        pokemon_species_details: Promise.all(
          y.pokemon_species_details.map((i) =>
            objectPromiseMixed({
              rate: i.rate,
              pokemon_species: findPromises["pokemonSpecies"]({
                name: i.pokemon_species.name,
        ...
        required_for_evolution: findManyPromises["pokemonSpecies"](
          mapFind(y.required_for_evolution, "name", "name"),
        ),
      })
        .then((res) => {
    ...
    nature: (y, next) => {
      ...
      objectPromiseMixed({
        name: y.name,
        decreased_stat: y.decreased_stat
          ? findPromises["stat"]({name: y.decreased_stat.name})
          : y.decreased_stat,
        increased_stat: y.increased_stat
          ? findPromises["stat"]({name: y.increased_stat.name})
          : y.increased_stat,
        hates_flavor: y.hates_flavor
          ? findPromises["berryFlavor"]({name: y.hates_flavor.name})
          : y.hates_flavor,
        likes_flavor: y.likes_flavor
          ? findPromises["berryFlavor"]({name: y.likes_flavor.name})
          : y.likes_flavor,
        pokeathlon_stat_changes: Promise.all(
          y.pokeathlon_stat_changes.map((i) =>
            objectPromiseMixed({
              max_change: i.max_change,
              pokeathlon_stat: findPromises["pokeathlonStat"]({
                name: i.pokeathlon_stat.name,
              }),
            }),
          ),
        ),
        move_battle_style_preferences: Promise.all(
          y.move_battle_style_preferences.map((i) =>
            objectPromiseMixed({
              low_hp_preference: i.low_hp_preference,
              high_hp_preference: i.high_hp_preference,
              move_battle_style: findPromises["moveBattleStyle"]({
                name: i.move_battle_style.name,
              }),
            }),
          ),
        ),
    ...
    "pokeathlon-stat": (y, next) => {
      objectPromiseMixed({
        ...
        affecting_natures: objectPromiseMixed({
          increase: Promise.all(
            y.affecting_natures.increase.map((i) =>
              objectPromiseMixed({
                max_change: i.max_change,
          ...
          decrease: Promise.all(
            y.affecting_natures.decrease.map((i) =>
              objectPromiseMixed({
                max_change: i.max_change,
                nature: findPromises["nature"]({name: i.nature.name}),
              }),
    ...
    pokemon: (y, next) => {
      ...
      objectPromiseMixed({
        ...
        forms: findManyPromises["pokemonForm"](
          mapFind(y.forms, "name", "name"),
        ),
        game_indices: Promise.all(
          y.game_indices.map((i) => {
            return objectPromiseMixed({
        ...
        location_area_encounters: Promise.all(
          y.location_area_encounters.map((i) =>
            objectPromiseMixed({
              location_area: findPromises["locationArea"]({
                name: i.location_area.name,
              }),
              version_details: Promise.all(
                i.version_details.map((k) =>
                  objectPromiseMixed({
        ...
        moves: Promise.all(
          y.moves.map((i) =>
            objectPromiseMixed({
              ...
              version_group_details: Promise.all(
                i.version_group_details.map((k) =>
                  objectPromiseMixed({
                    move_learn_method: findPromises["moveLearnMethod"]({
                      name: k.move_learn_method.name,
                    }),
                    version_group: findPromises["versionGroup"]({
                      name: k.version_group.name,
                    }),
    ...
    "pokemon-shape": (y, next) => {
      objectPromiseMixed({
        ...
        names: populateNames(y.names),
        pokemon_species: populateSpecies(y.pokemon_species),
        awesome_names: Promise.all(
          y.awesome_names.map((i) =>
            objectPromiseMixed({
    ...
    "pokemon-species": (y, next) => {
      objectPromiseMixed({
        ...
        pokedex_numbers: Promise.all(
          y.pokedex_numbers.map((i) =>
            objectPromiseMixed({
              entry_number: i.entry_number,
        ...
        egg_groups: findManyPromises["eggGroup"](
          mapFind(y.egg_groups, "name", "name"),
        ),
        color: findPromises["pokemonColor"]({name: y.color.name}),
        shape: findPromises["pokemonShape"]({name: y.shape.name}),
        evolves_from_species: y.evolves_from_species
          ? findPromises["pokemonSpecies"]({name: y.evolves_from_species.name})
          : y.evolves_from_species,
        evolution_chain: findPromises["evolutionChain"]({
          pokeapi_id: getApiId(y.evolution_chain.url),
        }),
        habitat: y.habitat
          ? findPromises["pokemonHabitat"]({name: y.habitat.name})
          : y.habitat,
        generation: findPromises["generation"]({name: y.generation.name}),
        names: populateNames(y.names),
        pal_park_encounters: Promise.all(
          y.pal_park_encounters.map((i) =>
            objectPromiseMixed({
              base_score: i.base_score,
              rate: i.rate,
    ...
    stat: (y, next) => {
      objectPromiseMixed({
        ...
        affecting_moves: objectPromiseMixed({
          increase: Promise.all(
            y.affecting_moves.increase.map((i) =>
              objectPromiseMixed({
                change: i.change,
                move: findPromises["move"]({name: i.move.name}),
              }),
          ...
          decrease: Promise.all(
            y.affecting_moves.decrease.map((i) =>
              objectPromiseMixed({
                change: i.change,
                move: findPromises["move"]({name: i.move.name}),
              }),
        ...
        affecting_natures: objectPromiseMixed({
          increase: findManyPromises["nature"](
            mapFind(y.affecting_natures.increase, "name", "name"),
          ),
          decrease: findManyPromises["nature"](
            mapFind(y.affecting_natures.decrease, "name", "name"),
          ),
        }),
        characteristics: findManyPromises["characteristic"](
          mapFind(y.characteristics, "name", "name"),
        ),
        move_damage_class: y.move_damage_class
          ? findPromises["moveDamageClass"]({name: y.move_damage_class.name})
          : y.move_damage_class,
        names: populateNames(y.names),
      })
        .then((res) => {
    ...
    type: (y, next) => {
      objectPromiseMixed({
        ...
        damage_relations: objectPromiseMixed({
          half_damage_from: findManyPromises["type"](
            mapFind(y.damage_relations.half_damage_from, "name", "name"),
          ),
          no_damage_from: findManyPromises["type"](
            mapFind(y.damage_relations.no_damage_from, "name", "name"),
          ),
          half_damage_to: findManyPromises["type"](
            mapFind(y.damage_relations.half_damage_to, "name", "name"),
          ),
          double_damage_from: findManyPromises["type"](
            mapFind(y.damage_relations.double_damage_from, "name", "name"),
          ),
          no_damage_to: findManyPromises["type"](
            mapFind(y.damage_relations.no_damage_to, "name", "name"),
          ),
          double_damage_to: findManyPromises["type"](
            mapFind(y.damage_relations.double_damage_to, "name", "name"),
          ),
        }),
        game_indices: Promise.all(
          y.game_indices.map((i) =>
  ...
  const findPromises = (() =>
    Object.keys(models).reduce(
      (accu, i) => ({
        [i]: (filter, projection = "_id") =>
          models[i].findOne(filter, projection),
        ...accu,
      }),
      {},
    ))();
  ...
  const findManyPromises = (() =>
    Object.keys(models).reduce(
      (accu, i) => ({
        [i]: async (filters, projection = "_id") =>
          await Promise.all(
            filters.map((filter) => models[i].findOne(filter, projection)),
          ),
        ...accu,
      }),
      {},
  ...
  const populateNames = async (names) =>
    await Promise.all(
      names.map((i) =>
        objectPromiseMixed({
          language: findPromises["language"]({name: i.language.name}),
  ...
  const populateDescriptions = async (descriptions) =>
    await Promise.all(
      descriptions.map((i) =>
        objectPromiseMixed({
          language: findPromises["language"]({name: i.language.name}),
  ...
  const populateSpecies = async (species) =>
    await findManyPromises["pokemonSpecies"](mapFind(species, "name", "name"));

  var funcs = Object.keys(populateType).map((key) => (next) => {
    async.waterfall(
      [
        (waterfallCallback) =>
          db.collection(key).find({}).toArray(waterfallCallback),
        (items, waterfallCallback) =>
          async.each(items, populateType[key], waterfallCallback),
      ],
      next,
    );
  });
    ...
    [
      ...
      (seriesCallback) => {
        async.waterfall(
          [
            (waterfallCallback) =>
              db.collection("move").find().toArray(waterfallCallback),
            (items, waterfallCallback) =>
              async.eachSeries(items, populateType["move"], waterfallCallback),
          ],
          seriesCallback,
        );
      },

...
mongo.connect(url, (err, db) => {
    ...
    async.each(routes, (route, next) => {
      ...
      async.parallel({
        pokeapi: (parallelCallback) => db.collection(pokeapi).find({}).toArray(parallelCallback),
        pokeql: (parallelCallback) => models[pokeql].find({}).exec(parallelCallback),
      }, (err, res) => {
        if(res.pokeapi.length !== res.pokeql.length) console.log("Not Equal", `pokeapi ${pokeapi}: ${res.pokeapi.length}`, `pokeql ${pokeql}: ${res.pokeql.length}`)
        next(err)
      })
    }, (err) => {
